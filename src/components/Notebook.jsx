...
//   child  - progressive disclosure: hints revealed one level at a time, self-explain loop.
//   parent - full diagnostic: transcription, error type and every hint up front, so a
//            parent can see the whole picture. The self-explain drill stays child-only.
export default function Notebook({ result, question, view = "child", onSaveMistake, saved }) {
  const isParent = view === "parent";
  const [hintLevel, setHintLevel] = useState(isParent ? 4 : 0);
  ...
  const shownHints = isParent ? hints : hints.slice(0, hintLevel);

  return (
    <section style={S.notebook} className="sc-reveal">
      {/* Accent rail carries the verdict colour. */}
      <div style={{ ...S.margin, background: pen.color }} />
      <div style={S.page}>
        <div style={{ ...S.pen, color: pen.color }}>{pen.text}</div>

        {result.chapter && <Row label="Chapter" body={result.chapter} />}
        {result.read_back && <Row label="What I read from your photo" body={result.read_back} />}
        ...
        {isParent && result.error_type && <Row label="Error type" body={result.error_type} accent={C.red} />}
        ...
        {result.verdict === "error_found" && hints.length > 0 && (
          <div style={{ marginBottom: 10 }}>
            <div style={{ ...S.rowLabel, color: C.ink }}>{isParent ? "Hints your child can unlock" : "Hints - try to fix it yourself first"}</div>
            {shownHints.map((h, i) => (
              <div key={i} style={hintCard}>
                <b>Hint {i + 1}{i === 3 ? " (worked example, different numbers)" : ""}:</b> {h}
              </div>
            ))}
            {!isParent && hintLevel < Math.min(4, hints.length) && (
        ...
        {result.verdict === "error_found" && result.self_explanation_prompt && !isParent && (
          <div style={{ marginBottom: 10 }}>
            ...
            {selfExpRes && (
              <div style={{ ...hintCard, borderColor: selfExpRes.understood ? C.green : C.amber, marginTop: 8 }}>
...
const hintCard = { background: C.surface, border: `1.5px solid ${C.border}`, borderRadius: 8, padding: "8px 10px", fontSize: 13.5, lineHeight: 1.45, marginBottom: 6 };
const hintBtn = { padding: "8px 14px", background: C.surface, border: `1.5px dashed ${C.ink}`, color: C.ink, fontWeight: 700, fontSize: 13, borderRadius: 8, cursor: "pointer" };
